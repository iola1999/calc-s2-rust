...
pub fn new_buffer() -> *const u8 {
    ...
    global_buffer_storage
        .buffer_map
        .insert(String::from("test"), buffer);
    ptr
}
...
pub fn get_buffer() -> *const u8 {
    ...
    // }
    if let Some(buffer) = global_buffer_storage.buffer_map.get("test") {
        return buffer.as_ptr();
    } else {
        return Vec::new().as_ptr();
    }
        ...
        for i in 1..width - 1 {
            for j in 1..height - 1 {
                ...
                for x in 0..3 {
                    // 取前后左右共9个格子
                    for y in 0..3 {
                        newR += buffer[width * (j + y - 1) * 4 + (i + x - 1) * 4 + 0] as i32
                            * kernel[y * 3 + x]
                            / kernel_length;
                        newG += buffer[width * (j + y - 1) * 4 + (i + x - 1) * 4 + 1] as i32
                            * kernel[y * 3 + x]
                            / kernel_length;
                        newB += buffer[width * (j + y - 1) * 4 + (i + x - 1) * 4 + 2] as i32
                            * kernel[y * 3 + x]
                            / kernel_length;
                    }
                }
